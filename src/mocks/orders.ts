...
export const getOrdersTableData = async (
  locale: string,
  key: string,
  value: string
  ...
  const orders = MockOrders.filter((order: any) => order[key] === value).map(
    (order: Order) => {
      return {
        ...
        totalUnitCount: order.totalUnitCount || 0,
        delivery: order.deliveryMode?.name || '',
        totalPrice: localizeCurrency(locale, order.totalPrice.value) || '',
        purchaseOrderNumber: order.purchaseOrderNumber,
        costCenterCode: order.costCenter?.code || '',
