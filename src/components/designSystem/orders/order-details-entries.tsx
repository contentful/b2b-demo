...
export const OrderDetailsEntries = (props: any) => {
  const { details } = props;
  const { status } = details;

  return (
    <div className='flex flex-col gap-2 h-1/3 w-full'>
      <OrderDetailsEntriesLabels />
      <div className='flex flex-col gap-2 h-1/3 w-full'>
        {details.entries.map((entry: OrderEntry) => {
          return (
            <OrderDetailsEntry key={entry.entryNumber} {...{ entry, status }} />
          );
        })}
      </div>
    </div>
...
const OrderDetailsEntriesLabels = () => {
...
const OrderDetailsEntry = (props: any) => {
